...
def gen_all_tables():
    ...
    for n in ns:
        archivo = f'./nuevas_poblaciones/jsons/tabla_{n}_nuevas_pob.json'
        try:
            datos = cargar_datos(archivo)
            tabla_bias = generar_tabla_latex("bias", datos, titulos[n])
            tabla_rmse = generar_tabla_latex("rmse", datos, titulos[n])

            with open(f'./nuevas_poblaciones/tablas/tabla_{n}.txt', 'w') as f:
                f.write(tabla_bias)
                f.write("\n\n")      
                f.write(tabla_rmse)
...
gen_all_tables()
ruta_carpeta = "./nuevas_poblaciones/tablas"
archivo_salida = "./nuevas_poblaciones/tablas/tablas_simulacion_altered.txt"
