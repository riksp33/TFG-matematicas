...
MC = 1000
lista_json = list()
lista_json['header'] = 'Simulacion para nueva población 3'
...
for (auc in AUCs) {
  ...
  for (n in ns) {
    for (i in 1:MC) {
      ...
      estimaciones_param[i] = EtaBinormal(casos = Y, control = X)
...
dir_path = here('nuevas_poblaciones', 'jsons')
full_path = file.path(dir_path,'tabla_III_nuevas_pob.json' )
