...
for (i in 1:length(aucs)) {
  ...
  auc = aucs[i]
  mu_x = ObtenerMux(auc, sqrt(0.09), sqrt(0.25), 2.5 )
  eta = eta_poblacional_IV(mu_x)
  eta_sd = eta / (eta +1)
  eta_trans = log(eta + 1) / (log(eta + 1) + 1)

  
  resultado[i,] = c(mu_x, auc, eta, eta_sd, eta_trans)
  cat(' Media: ', mu_x ,'\n', 'AUC: ', auc, '\n')
  cat(' eta: ', eta , '\n')
...
df
write_xlsx(df, 'nueva_estandarizacion_IV.xlsx')
