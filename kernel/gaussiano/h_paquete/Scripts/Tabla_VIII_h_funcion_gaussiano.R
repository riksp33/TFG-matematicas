...
SimulateBiasRMSE = function(AUCs , tamaños){
  lista_json = list()
  lista_json['header'] = 'Simulación de kernel de Faraggi, h paquete, gaussiano tabla 8'
    for (auc in AUCs) {
    resultado_auc = list()
    ...
    ##############################################################
    eta_pob_kernel = eta_poblacional_VIII(mu_x, mu_x + 4, 1, sqrt(5), 0.5)
    cat('===================================================================\n')
  ...
  full_path = file.path(dir_path,'tabla_VIII_kernel_h_funcion_gaussiano.json' )
