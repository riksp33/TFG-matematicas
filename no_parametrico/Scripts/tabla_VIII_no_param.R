...
SimulateBiasRMSE = function(AUCs , tamaños){
  lista_json = list()
  lista_json['header'] = 'Simulación de la tabla 8 de Faraggi'
  ...
  for (auc in AUCs) {
    ...
    eta_pob_empirico = eta_poblacional_VIII(mu_x, mu_x + 4, 1, sqrt(5), 0.5)
    cat('===================================================================\n')
  ...
  full_path = file.path(dir_path,'tabla_VIII_no_param.json' )
