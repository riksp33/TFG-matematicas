...
function potencia_escenario_10()
    Random.seed!(1)
    B = 500
    MC = 1000
    alpha = 0.05
    ...
    results = Dict{Any, Any}("title" => "Resultados de nivel del test para distintos tamaños muestrales. Escenario 10")
    ...
    for n in n_values
        ...
        @threads for i in 1:MC
            ...
            for b in 1:B
                ...
                kernel_opt_bootstrap[b] = eta_kernel(controles_b, casos_b, "optimo")
                kernel_hscv_bootstrap[b] = eta_kernel(controles_b, casos_b, "hscv")
            end
    ...
    output_path = joinpath(output_dir, "potencias_10.json")
