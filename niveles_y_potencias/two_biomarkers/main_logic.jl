...
function scenario_1_two_biomarkers()
    Random.seed!(1)
    B = 250
    MC = 100
    alpha = 0.05
    n_values = [20, 50, 100]
    results = Dict{Any, Any}("title" => "Resultados de potencia del test 2 marcadores para distintos tamaños muestrales. Escenario 1")
    ...
    output_dir = joinpath(@__DIR__, "..", "two_biomarkers", "jsons")
    if !isdir(output_dir)
    mkpath(output_dir)
    end

    output_path = joinpath(output_dir, "scenario_1.json")
